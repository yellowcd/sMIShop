...
bool OpticalFlow::SaveOpticalFlow(const DImage& flow, const char* filename)
{
	...
	for(int i =0;i<flow.npixels();i++)
	{
		foo.data()[i*2] = (unsigned short int)(__min(__max(flow.data()[i*2],-200),200)+200)*160;
		foo.data()[i*2+1] = (unsigned short int)(__min(__max(flow.data()[i*2+1],-200),200)+200)*160;
	}
