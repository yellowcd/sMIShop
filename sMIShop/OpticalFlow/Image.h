...
bool Image<T>::imresize(double ratio)
{
	...
	DstWidth=(int)((double)imWidth*ratio);
	DstHeight=(int)((double)imHeight*ratio);
...
bool Image<T>::loadImage(ifstream& myfile)
{
	...
	if(_strcmpi(type,"uint16")==0)
		sprintf_s(type,"unsigned short");
	if(_strcmpi(type,"uint32")==0)
		sprintf_s(type,"unsigned int");
...
void Image<T>::warpImageBicubicRef(const Image<T>& ref,Image<T>& output,
	const Image<T1>& imdx,const Image<T1>& imdy,const Image<T1>& imdxdy,
	const Image<T2>& vx,const Image<T2>& vy) const
{
	T* pIm = pData;
	const T1* pImDx = imdx.data();
